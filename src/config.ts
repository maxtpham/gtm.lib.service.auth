...
export function normalizeAuth(config: entities.IAuthConfig): entities.IAuthConfig {
    if (!config.jwt) config.jwt = <entities.IJwtConfig>{};
    ...
    if (!config.swagger) config.swagger = <entities.ISwaggerConfig>{};
